...
def create_leave_request(
    leave_request: Leavings.LeaveRequestCreate,
    db=Depends(get_db),
...
def update_leave_request(
    leave_request_id: int,
    leave_request: Leavings.LeaveRequestUpdate,
    db=Depends(get_db),
    current_user=Depends(get_current_active_user)  # Enforces JWT token authentication
):
...
def get_team_leave_requests(
    ...
    if user_role is None and user_role != current_user.username:
        raise HTTPException(status_code=status.HTTP_403_FORBIDDEN, detail="You do not have permission to view this information")
